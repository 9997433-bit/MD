...
def test_no_empty_status():
    all_entries = HW_ENTRIES + BIT_ENTRIES + SIG_ENTRIES + USB_ENTRIES + REF_ENTRIES + ARCH_ENTRIES
...
def test_minimum_entry_count():
    total = len(HW_ENTRIES) + len(BIT_ENTRIES) + len(SIG_ENTRIES) + len(USB_ENTRIES) + len(REF_ENTRIES) + len(ARCH_ENTRIES)
    assert total >= 180, f"Only {total} entries, need >= 180"
