...
def test_no_empty_status():
    all_entries = HW_ENTRIES + BIT_ENTRIES + SIG_ENTRIES + USB_ENTRIES + REF_ENTRIES + ARCH_ENTRIES + LEARN_ENTRIES
...
def test_minimum_entry_count():
    total = len(HW_ENTRIES) + len(BIT_ENTRIES) + len(SIG_ENTRIES) + len(USB_ENTRIES) + len(REF_ENTRIES) + len(ARCH_ENTRIES) + len(LEARN_ENTRIES)
    assert total >= 200, f"Only {total} entries, need >= 200"
