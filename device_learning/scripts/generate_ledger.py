...
def normalize_usb(entry: dict) -> dict:
  if "description" in entry and entry.get("layer", "").lower() == "usb":
    return entry
  missing = entry.get("missing") or ""
  boundary = entry.get("boundary") or missing
  evidence = entry.get("source") if entry.get("source") not in (None, "none") else None
  return {
    "identifier": entry["identifier"],
    "layer": "usb",
    "module": entry["layer"].lower(),
...
def build_coverage(entries: list[dict]) -> dict:
    ...
    stop = {
        ...
        "2_all_layers_present": all(
            ...
            for layer in ("hw", "bit", "signal", "usb")
        ),
...
def main() -> None:
    ...
    for script in ("parse_bit_header.py", "parse_bitstream.py"):
        if script == "parse_bit_header.py":
            subprocess.run(
                [sys.executable, str(ROOT / "scripts" / script), str(ROOT / "firmware" / "device.bit"),
                 "-o", str(MANIFESTS / "bitstream_meta.json")],
                check=True,
            )
        else:
    ...
    all_entries = HW_ENTRIES + BIT_ENTRIES + SIG_ENTRIES + usb_norm
    ledger = {
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "declaration": "目录完整 ≠ 厂商等价 ≠ 掌握运行行为",
        "catalogs": {
